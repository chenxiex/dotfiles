...
# Escape percent signs before storing Git text in PROMPT_SUBST variables.
function _prompt_escape() {
    print -r -- "${1//\%/%%}"
}

# Refresh the local Git branch/ref and dirty marker. No remote/upstream checks.
function _prompt_update_git_info() {
    _PROMPT_GIT=

    command git rev-parse --is-inside-work-tree >/dev/null 2>&1 || return

    local ref escaped_ref dirty_marker

    ref="$(command git symbolic-ref --quiet --short HEAD 2>/dev/null)" \
        || ref="$(command git rev-parse --short HEAD 2>/dev/null)" \
        || return
    escaped_ref="$(_prompt_escape "$ref")"

    if [[ -n $(command git status --porcelain --ignore-submodules=dirty 2>/dev/null) ]]; then
        dirty_marker="*"
    fi

    _PROMPT_GIT=" %F{242}${escaped_ref}%F{yellow}${dirty_marker}%f"
}
